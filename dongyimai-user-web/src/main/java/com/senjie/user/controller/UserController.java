...
import com.senjie.utils.PhoneFormatCheckUtils;
...
public class UserController {
    ...
    public Result add (@RequestBody TbUser user, String code) {
        boolean b = userService.determineCode(user.getPhone(), code);
        if (b == false) {
            return new Result(false, "The verification code is incorrect");
        }
        try {
    ...
    public Result sendCode (String phone) {
        ...
        if (!PhoneFormatCheckUtils.isPhoneLegal(phone)) {
            return new Result(false, "phone number  incorrectly entered");
        }
        try {
            userService.createCode(phone);
            return new Result(true, "SMS sent successfully!");
        } catch (Exception e) {
            e.printStackTrace();
            return new Result(false, "SMS failed to send!");
        }
